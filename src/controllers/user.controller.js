...
const generateAccessAndRefreshTokens = async(userId) => {
    try {
        ...
        throw new ApiError(500, "Something went while generating and accessing tokens")
    }
}
...
const registerUser = asyncHandler( async (req, res) => {
    ...
    //validation all are given and in correct format=> not empty
    //check if user already exist:(username, email)
    //check for images, check for avatar
    //upload them to cloudinary
    //check on cloudinary whether avatar is uploaded or not
    //create user Object(mongo db mei joo data bhejunga nosql database hai object hiii banaye aaur upload kia jaate hai, create entry in db.)
...
const loginUser = asyncHandler(async (req, res) => {
    ...
    //(!(username || email)) agar or chahiye too
    if(!username && !email) {
